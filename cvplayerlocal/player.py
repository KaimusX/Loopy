...
def _show_black_and_wait(self):
    self.state = 'paused'
    cv2.namedWindow('LOOPY - Video Player', cv2.WINDOW_KEEPRATIO)
    if self.fullscreen: cv2.setWindowProperty('LOOPY - Video Player', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
    cv2.imshow('LOOPY - Video Player', numpy.zeros((512, 512, 3), dtype = "uint8"))
...
class VideoPlayer:
    def __init__(self, filename,
        ...
        opts = {'sync': 'audio', 'paused': paused, 'volume': volume, 't': t+1, 'ss': ss, 'loop': 0}
                ...
                if val != 'eof' or frame is not None:
                    if self.fullscreen is True:
                        cv2.setWindowProperty(
                            'LOOPY - Video Player', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
                    elif self.fullscreen is False:
                        if x != '' and y != '':
                            cv2.resizeWindow('LOOPY - Video Player', x, y)
                        cv2.setWindowProperty(
                            'LOOPY - Video Player', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_NORMAL)
                    if frame is not None:
                        x, y = image.get_size()
                        data = image.to_bytearray()[0]
                        image =  Image.frombytes("RGB", (x, y), bytes(data))
                        image = cv2.cvtColor(numpy.array(image), cv2.COLOR_RGB2BGR)
                        cv2.imshow('LOOPY - Video Player', image)
                        del image
                else:
                    break
